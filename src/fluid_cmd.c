...
fluid_handle_set(fluid_cmd_handler_t* handler, int ac, char** av, fluid_ostream_t out)
{
  ...
  if (fluid_is_number(av[1])) {
    if (FLUID_STRCHR(av[1], '.') != NULL) {
      fluid_synth_setnum(handler->synth, av[0], atof(av[1]));
    } else {
      fluid_synth_setint(handler->synth, av[0], atoi(av[1]));
    }
  } else {
    fluid_synth_setstr(handler->synth, av[0], av[1]);
  }
...
fluid_handle_get(fluid_cmd_handler_t* handler, int ac, char** av, fluid_ostream_t out)
{
  ...
  case FLUID_NUM_TYPE: {
    double value;
    fluid_synth_getnum(handler->synth, av[0], &value);
  ...
  case FLUID_INT_TYPE: {
    int value;
    fluid_synth_getint(handler->synth, av[0], &value);
  ...
  case FLUID_STR_TYPE: {
    char* s;
    fluid_synth_getstr(handler->synth, av[0], &s);
...
fluid_cmd_handler_t* new_fluid_cmd_handler(fluid_synth_t* synth, fluid_midi_router_t* router)
{
  ...
  handler->commands = new_fluid_hashtable(fluid_cmd_handler_delete);
...
int fluid_cmd_handler_register(fluid_cmd_handler_t* handler, fluid_cmd_t* cmd)
{
  ...
  fluid_hashtable_insert(handler->commands, copy->name, copy, 0);
  return 0;
}
...
int fluid_cmd_handler_handle(fluid_cmd_handler_t* handler, int ac, char** av, fluid_ostream_t out)
{
  ...
  if (fluid_hashtable_lookup(handler->commands, av[0], &vp, NULL)
